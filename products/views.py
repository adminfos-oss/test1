...
class ProductViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        # Публичный просмотр ВСЕГО для list и retrieve
        # if self.action in ['list', 'retrieve']:
        #     # print(f"[DEBUG] {self.action.upper()} — возвращаю ВСЕ продукты")
        #     return Product.objects.all()

        # user = self.request.user
        # if user.is_authenticated:
        #     # print(f"[DEBUG] Авторизован: {user.email}, показываю только свои продукты")
        #     return Product.objects.filter(created_by=user)
        # return Product.objects.none()  # Анонимам ничего, кроме list/retrieve
